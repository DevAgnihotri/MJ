...
const Config = {
    ...
    faceNumbers: [1, 2, 3, 6, 5, 4, 1, 2, 3, 6, 5, 6, 1, 2, 3, 4, 5, 4, 1, 2],  // 20 faces but only values 1-6
...
const CRTShader = {
    ...
    fragmentShader: `
        ...
        float getBrightness(vec3 color) {
            return dot(color, vec3(0.4, 0.1, 0.7)); // Weight purple higher
        }
...
const D20 = {
    ...
    init: function (scene) {
        // Use the original icosahedron (20-sided) geometry
        ...
        RendererModule.pixelateShader = {
            ...
            fragmentShader: `
              ...
              void main() {
                  ...
                  gl_FragColor = texture2D(tDiffuse, coord) * vec4(0.5, 0.3, 1.0, 1.0);
              }
            `
        };
...
const LabelModule = {
    ...
    flashNumber: function (faceNumber, labels) {
        // First, let's ensure we reset any previous flash
        ...
        // Find the label that is most directly facing the camera
        let bestLabel = null;
        let highestDot = -1;
        
        const cameraPosition = new THREE.Vector3();
        RendererModule.camera.getWorldPosition(cameraPosition);
        
        // Look through all labels to find the most visible one with our target number
        for (let i = 0; i < labels.length; i++) {
            const label = labels[i];
            // Skip labels that don't match our target number
            if (Config.faceNumbers[i] !== faceNumber) continue;
        ...
        // If we found a good face, highlight it
        if (bestLabel && highestDot > 0.1) {
            const sprite = bestLabel.children[0];
            if (sprite) {
                ...
                console.log("Flashing number:", faceNumber, "with visibility:", highestDot);
                return;
            }
        }
        
        // Fallback: If we didn't find a good visible face with our number,
        // just pick any face with our target number
        for (let i = 0; i < labels.length; i++) {
            if (Config.faceNumbers[i] === faceNumber) {
                const sprite = labels[i].children[0];
                if (sprite) {
                    LabelModule.currentFlashedSprite = sprite;
                    LabelModule.originalColor = sprite.material.color.clone();
                    sprite.material.color.setHex(Config.flashColor);
                    console.log("Flashing number (fallback):", faceNumber);
                    return;
                }
            }
        }
        
        // Emergency fallback: If somehow we still didn't highlight anything, 
        // just highlight a random face
        console.warn("Could not find face number:", faceNumber);
        const randomIndex = Math.floor(Math.random() * labels.length);
        const sprite = labels[randomIndex].children[0];
        if (sprite) {
            LabelModule.currentFlashedSprite = sprite;
            LabelModule.originalColor = sprite.material.color.clone();
            sprite.material.color.setHex(Config.flashColor);
            console.log("Flashing random number:", Config.faceNumbers[randomIndex]);
        }
    },
    ...
    getFaceFacingCamera: function (labels, camera) {
        ...
        for (let i = 0; i < labels.length; i++) {
            ...
            // Only consider faces that are somewhat facing the camera
            if (dot > 0.1) {
                visibleLabels.push({
                    index: i,
                    dot: dot,
                    number: Config.faceNumbers[i]
                });
        ...
        // If we have a clear winner with good visibility
        if (maxDot > 0.5) {
            return facingFaceNumber;
        }
        
        // If we have any visible faces but none are clearly dominant
        if (visibleLabels.length > 0) {
            // Sort by visibility (most visible first)
            visibleLabels.sort((a, b) => b.dot - a.dot);
            // Take one of the top 3 most visible faces (with some randomness)
            const topIndex = Math.min(2, visibleLabels.length - 1);
            const selectedIndex = Math.floor(Math.random() * (topIndex + 1));
            return visibleLabels[selectedIndex].number;
        }
        
        // Fallback: if no faces are sufficiently visible, return a random number from 1-6
